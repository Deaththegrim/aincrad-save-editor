//! UI translations. English is the base; other languages match the locales the
//! game itself ships. Only the static UI labels are translated (dynamic status /
//! error text stays English so it's easy to support). Community corrections to any
//! translation are welcome.

#[derive(Clone, Copy, PartialEq, Eq)]
pub enum Lang {
    En,
    Ja,
    Fr,
    De,
    Es,
    PtBr,
    It,
    Ru,
    ZhHans,
    Ko,
}

impl Lang {
    /// All languages, in menu order, with their native display name.
    pub const ALL: &'static [(Lang, &'static str)] = &[
        (Lang::En, "English"),
        (Lang::Ja, "日本語"),
        (Lang::Fr, "Français"),
        (Lang::De, "Deutsch"),
        (Lang::Es, "Español"),
        (Lang::PtBr, "Português (BR)"),
        (Lang::It, "Italiano"),
        (Lang::Ru, "Русский"),
        (Lang::ZhHans, "简体中文"),
        (Lang::Ko, "한국어"),
    ];

    pub fn code(self) -> &'static str {
        match self {
            Lang::En => "en",
            Lang::Ja => "ja",
            Lang::Fr => "fr",
            Lang::De => "de",
            Lang::Es => "es",
            Lang::PtBr => "pt-br",
            Lang::It => "it",
            Lang::Ru => "ru",
            Lang::ZhHans => "zh-hans",
            Lang::Ko => "ko",
        }
    }

    pub fn from_code(c: &str) -> Lang {
        Self::ALL.iter().map(|(l, _)| *l).find(|l| l.code() == c).unwrap_or(Lang::En)
    }
}

/// Every translatable static UI label.
pub struct S {
    pub open_save: &'static str,
    pub save: &'static str,
    pub apply_to_game: &'static str,
    pub reload: &'static str,
    pub working_copy: &'static str,
    pub unsaved: &'static str,
    pub thumbnails: &'static str,
    pub character: &'static str,
    pub copy_diagnostics: &'static str,
    pub language: &'static str,
    // categories
    pub cat_identity: &'static str,
    pub cat_face: &'static str,
    pub cat_hair: &'static str,
    pub cat_body: &'static str,
    pub cat_looks: &'static str,
    pub cat_backups: &'static str,
    // backups page
    pub backups_intro: &'static str,
    pub backup_load: &'static str,
    pub no_backups: &'static str,
    /// Row label: a backup of the game's live save file.
    pub backup_live: &'static str,
    /// Row label: a backup of the editor's working copy.
    pub backup_work: &'static str,
    // identity
    pub name: &'static str,
    pub gender: &'static str,
    pub male: &'static str,
    pub female: &'static str,
    pub voice: &'static str,
    /// Hint under the voice picker: voices aren't gender-locked (the game keys
    /// the Wwise switch on the voice name alone), so cross-gender picks work.
    pub voice_any_body: &'static str,
    /// Tooltip on the voice-preview play button.
    pub voice_preview: &'static str,
    // game mode (slot-level, Identity page)
    pub mode_title: &'static str,
    /// The permadeath checkbox label.
    pub death_game: &'static str,
    /// Note under it: what the flag is + when a change takes effect.
    pub death_game_note: &'static str,
    // face pickers
    pub nose: &'static str,
    pub eyebrows: &'static str,
    pub eye_shape: &'static str,
    pub eyes_iris: &'static str,
    pub head_shape: &'static str,
    // hair pickers
    pub hair: &'static str,
    pub mole: &'static str,
    pub freckles: &'static str,
    pub none: &'static str,
    // colours / body
    pub colours: &'static str,
    /// Toggle that reveals the game creator's own palette for a colour.
    pub game_palette: &'static str,
    /// Hover text on that toggle.
    pub game_palette_hint: &'static str,
    pub body_hidden: &'static str,
    /// Chest→neck coupling note: the game's BS_BOD_Chest morph reaches the neck
    /// base and there is no separate neck control (exe-verified) — shown so
    /// users stop reporting the neck change as an editor bug.
    pub body_neck_note: &'static str,
    // key screen
    pub set_key: &'static str,
    pub key_needs: &'static str,
    pub key_recover: &'static str,
    pub key_scanning: &'static str,
    pub save_key: &'static str,
    /// Top-bar control to return to the key screen (paste a new key / re-recover).
    pub change_key: &'static str,
    pub key_hint: &'static str,
    pub key_not_ship: &'static str,
    // looks
    pub looks_intro: &'static str,
    pub save_look_as: &'static str,
    pub save_look: &'static str,
    pub saved_looks: &'static str,
    pub no_looks: &'static str,
    pub apply: &'static str,
    pub delete: &'static str,
    pub open_to_begin: &'static str,
    /// Prefix for the top-bar update link, e.g. "Update available" → shown as
    /// "⬆ Update available v0.1.16".
    pub update_available: &'static str,
    /// Hover text on that link.
    pub update_hint: &'static str,
}

/// The active language's strings.
pub fn s(lang: Lang) -> &'static S {
    match lang {
        Lang::En => &EN,
        Lang::Ja => &JA,
        Lang::Fr => &FR,
        Lang::De => &DE,
        Lang::Es => &ES,
        Lang::PtBr => &PT,
        Lang::It => &IT,
        Lang::Ru => &RU,
        Lang::ZhHans => &ZH,
        Lang::Ko => &KO,
    }
}

const EN: S = S {
    open_save: "Open save…", save: "Save", apply_to_game: "Apply to game…", reload: "Reload",
    working_copy: "WORKING COPY — live save safe", unsaved: "unsaved",
    thumbnails: "Thumbnails", character: "Character", copy_diagnostics: "Copy diagnostics",
    language: "Language",
    cat_identity: "Identity", cat_face: "Face", cat_hair: "Hair & Extras", cat_body: "Body", cat_looks: "Looks",
    cat_backups: "Backups",
    backups_intro: "Every Save or Apply keeps a timestamped backup — of the working copy here in the editor's folder, and of the live save next to the game's save file. Load one to inspect or restore it: loading only fills the working copy; your live save is untouched until you Apply.",
    backup_load: "Load", no_backups: "No backups yet — they appear after your first Save or Apply.",
    backup_live: "game save", backup_work: "working copy",
    name: "Name", gender: "Gender", male: "Male", female: "Female", voice: "Voice",
    voice_any_body: "Voices aren't gender-locked — any voice works on any body.",
    voice_preview: "Play a sample of this voice — each press plays a different line.",
    mode_title: "Game mode",
    death_game: "Death game (permadeath)",
    death_game_note: "The permadeath choice from character creation. On: this character's death is meant to be final. Off: death is normal. Applies the next time the game loads this save.",
    nose: "Nose", eyebrows: "Eyebrows", eye_shape: "Eye shape", eyes_iris: "Eyes / iris", head_shape: "Head shape",
    hair: "Hair", mole: "Mole", freckles: "Freckles", none: "None",
    colours: "Colours",
    game_palette: "Game colours",
    game_palette_hint: "Pick from the character creator's own palette — click a swatch to match the game.",
    body_hidden: "Body sliders are capped to the game's safe range, so you can't warp your character into a broken mesh.",
    body_neck_note: "Chest also reshapes the neck base — the game has no separate neck control; the in-game creator behaves the same way.",
    set_key: "Set your AES key",
    key_needs: "The editor needs Echoes of Aincrad's pak AES key to read your save. Paste it below, or recover it from your running game.",
    key_recover: "Recover from running game", key_scanning: "Scanning…", save_key: "Save key",
    change_key: "Change key",
    key_hint: "Don't have the key? Launch Echoes of Aincrad, get into the world, then click \"Recover from running game\".",
    key_not_ship: "We never ship the key — this reads your own from your own game.",
    looks_intro: "Save this character's appearance as a named look, or apply a saved one. Looks are your own backups, separate from the game save.",
    save_look_as: "Save current look as", save_look: "Save look", saved_looks: "Saved looks",
    no_looks: "No looks saved yet.", apply: "Apply", delete: "Delete",
    open_to_begin: "Open a save to begin.",
    update_available: "Update available", update_hint: "A newer version of the editor is available — click to open the downloads page.",
};

const JA: S = S {
    open_save: "セーブを開く…", save: "保存", apply_to_game: "ゲームに適用…", reload: "再読み込み",
    working_copy: "作業コピー — 元のセーブは安全", unsaved: "未保存",
    thumbnails: "サムネイル", character: "キャラクター", copy_diagnostics: "診断情報をコピー",
    language: "言語",
    cat_identity: "基本情報", cat_face: "顔", cat_hair: "髪・その他", cat_body: "体", cat_looks: "ルック",
    cat_backups: "バックアップ",
    backups_intro: "保存や適用のたびに、タイムスタンプ付きバックアップが作られます（作業コピーはエディタのフォルダに、ゲームのセーブはセーブファイルの隣に）。読み込んで内容の確認や復元ができます。読み込みは作業コピーに入るだけで、「ゲームに適用」するまで実際のセーブには触れません。",
    backup_load: "読み込む", no_backups: "バックアップはまだありません。最初の保存または適用後に表示されます。",
    backup_live: "ゲームのセーブ", backup_work: "作業コピー",
    name: "名前", gender: "性別", male: "男性", female: "女性", voice: "ボイス",
    voice_any_body: "ボイスは性別に固定されません。どの体にも好きなボイスを使えます。",
    voice_preview: "このボイスのサンプルを再生します。押すたびに別のセリフが流れます。",
    mode_title: "ゲームモード",
    death_game: "デスゲーム（パーマデス）",
    death_game_note: "キャラクター作成時のパーマデス設定です。オン：このキャラクターの死は取り返しがつきません。オフ：通常の死亡扱いです。次にゲームがこのセーブを読み込んだ時から適用されます。",
    nose: "鼻", eyebrows: "眉", eye_shape: "目の形", eyes_iris: "瞳", head_shape: "頭の形",
    hair: "髪", mole: "ほくろ", freckles: "そばかす", none: "なし",
    colours: "カラー",
    game_palette: "ゲームの色",
    game_palette_hint: "キャラクタークリエイターのパレットから選択 — スウォッチをクリックするとゲームの色に合わせられます。",
    body_hidden: "体型スライダーはゲームの安全な範囲に制限されており、モデルが壊れる心配はありません。",
    body_neck_note: "「胸」は首の付け根の形にも影響します。ゲームに独立した首の設定はなく、ゲーム内キャラメイクでも同じ挙動です。",
    set_key: "AESキーを設定",
    key_needs: "セーブを読むにはEchoes of Aincradのpak AESキーが必要です。下に貼り付けるか、起動中のゲームから取得してください。",
    key_recover: "起動中のゲームから取得", key_scanning: "スキャン中…", save_key: "キーを保存",
    change_key: "キーを変更",
    key_hint: "キーがない場合はEchoes of Aincradを起動し、ゲーム内に入ってから「起動中のゲームから取得」を押してください。",
    key_not_ship: "キーは配布しません。ご自身のゲームからご自身のキーを読み取ります。",
    looks_intro: "現在の見た目を名前付きのルックとして保存、または保存済みを適用できます。ルックはゲームセーブとは別のあなた専用のバックアップです。",
    save_look_as: "現在のルックを保存", save_look: "ルックを保存", saved_looks: "保存済みルック",
    no_looks: "まだルックがありません。", apply: "適用", delete: "削除",
    open_to_begin: "セーブを開いて始めましょう。",
    update_available: "アップデートあり", update_hint: "エディターの新しいバージョンがあります。クリックでダウンロードページを開きます。",
};

const FR: S = S {
    open_save: "Ouvrir une sauvegarde…", save: "Enregistrer", apply_to_game: "Appliquer au jeu…", reload: "Recharger",
    working_copy: "COPIE DE TRAVAIL — sauvegarde protégée", unsaved: "non enregistré",
    thumbnails: "Vignettes", character: "Personnage", copy_diagnostics: "Copier le diagnostic",
    language: "Langue",
    cat_identity: "Identité", cat_face: "Visage", cat_hair: "Cheveux & Extras", cat_body: "Corps", cat_looks: "Looks",
    cat_backups: "Backups",
    backups_intro: "Chaque Enregistrer ou Appliquer conserve une sauvegarde horodatée — de la copie de travail dans le dossier de l'éditeur, et de la sauvegarde du jeu à côté de son fichier. Chargez-en une pour l'inspecter ou la restaurer : le chargement ne remplit que la copie de travail ; votre sauvegarde réelle n'est pas touchée avant d'Appliquer.",
    backup_load: "Charger", no_backups: "Aucun backup pour l'instant — ils apparaissent après le premier enregistrement ou application.",
    backup_live: "sauvegarde du jeu", backup_work: "copie de travail",
    name: "Nom", gender: "Genre", male: "Homme", female: "Femme", voice: "Voix",
    voice_any_body: "Les voix ne sont pas liées au genre — n'importe quelle voix fonctionne sur n'importe quel corps.",
    voice_preview: "Écouter un échantillon de cette voix — chaque pression joue une réplique différente.",
    mode_title: "Mode de jeu",
    death_game: "Death game (mort permanente)",
    death_game_note: "Le choix de mort permanente fait à la création du personnage. Activé : la mort de ce personnage est définitive. Désactivé : mort normale. Prend effet au prochain chargement de la sauvegarde.",
    nose: "Nez", eyebrows: "Sourcils", eye_shape: "Forme des yeux", eyes_iris: "Yeux / iris", head_shape: "Forme du visage",
    hair: "Cheveux", mole: "Grain de beauté", freckles: "Taches de rousseur", none: "Aucun",
    colours: "Couleurs",
    game_palette: "Couleurs du jeu",
    game_palette_hint: "Choisissez dans la palette du créateur de personnage — cliquez sur un échantillon pour reprendre la couleur du jeu.",
    body_hidden: "Les curseurs de morphologie sont limités à la plage sûre du jeu, pour ne pas déformer votre personnage.",
    body_neck_note: "Le curseur Poitrine remodèle aussi la base du cou — le jeu n'a pas de réglage de cou séparé ; le créateur du jeu fait pareil.",
    set_key: "Définir votre clé AES",
    key_needs: "L'éditeur a besoin de la clé AES pak d'Echoes of Aincrad pour lire votre sauvegarde. Collez-la ci-dessous ou récupérez-la depuis votre jeu en cours.",
    key_recover: "Récupérer depuis le jeu", key_scanning: "Analyse…", save_key: "Enregistrer la clé",
    change_key: "Changer la clé",
    key_hint: "Pas de clé ? Lancez Echoes of Aincrad, entrez dans le monde, puis cliquez sur « Récupérer depuis le jeu ».",
    key_not_ship: "Nous ne fournissons jamais la clé : elle est lue depuis votre propre jeu.",
    looks_intro: "Enregistrez l'apparence de ce personnage comme un look nommé, ou appliquez-en un. Les looks sont vos propres sauvegardes, séparées du jeu.",
    save_look_as: "Enregistrer le look actuel", save_look: "Enregistrer le look", saved_looks: "Looks enregistrés",
    no_looks: "Aucun look enregistré.", apply: "Appliquer", delete: "Supprimer",
    open_to_begin: "Ouvrez une sauvegarde pour commencer.",
    update_available: "Mise à jour disponible", update_hint: "Une version plus récente de l'éditeur est disponible — cliquez pour ouvrir la page de téléchargement.",
};

const DE: S = S {
    open_save: "Spielstand öffnen…", save: "Speichern", apply_to_game: "Ins Spiel übernehmen…", reload: "Neu laden",
    working_copy: "ARBEITSKOPIE — echter Spielstand sicher", unsaved: "ungespeichert",
    thumbnails: "Vorschaubilder", character: "Charakter", copy_diagnostics: "Diagnose kopieren",
    language: "Sprache",
    cat_identity: "Identität", cat_face: "Gesicht", cat_hair: "Haare & Extras", cat_body: "Körper", cat_looks: "Looks",
    cat_backups: "Backups",
    backups_intro: "Jedes Speichern oder Anwenden legt ein Backup mit Zeitstempel an — von der Arbeitskopie im Editor-Ordner und vom Spielstand neben der Spielstand-Datei. Lade eines zum Prüfen oder Wiederherstellen: Das Laden füllt nur die Arbeitskopie; dein Spielstand bleibt unberührt, bis du anwendest.",
    backup_load: "Laden", no_backups: "Noch keine Backups — sie erscheinen nach dem ersten Speichern oder Anwenden.",
    backup_live: "Spielstand", backup_work: "Arbeitskopie",
    name: "Name", gender: "Geschlecht", male: "Männlich", female: "Weiblich", voice: "Stimme",
    voice_any_body: "Stimmen sind nicht ans Geschlecht gebunden — jede Stimme funktioniert mit jedem Körper.",
    voice_preview: "Hörprobe dieser Stimme abspielen — jeder Druck spielt eine andere Zeile.",
    mode_title: "Spielmodus",
    death_game: "Death Game (Permadeath)",
    death_game_note: "Die Permadeath-Wahl aus der Charaktererstellung. An: Der Tod dieses Charakters ist endgültig gemeint. Aus: normaler Tod. Gilt ab dem nächsten Laden des Spielstands.",
    nose: "Nase", eyebrows: "Augenbrauen", eye_shape: "Augenform", eyes_iris: "Augen / Iris", head_shape: "Kopfform",
    hair: "Haare", mole: "Muttermal", freckles: "Sommersprossen", none: "Keine",
    colours: "Farben",
    game_palette: "Spielfarben",
    game_palette_hint: "Aus der Palette des Charaktereditors wählen — ein Klick auf ein Farbfeld übernimmt die Spielfarbe.",
    body_hidden: "Die Körperform-Regler sind auf den sicheren Bereich des Spiels begrenzt, damit dein Charakter nicht verzerrt wird.",
    body_neck_note: "Der Brust-Regler formt auch den Halsansatz mit — das Spiel hat keinen separaten Hals-Regler; der Spiel-Editor verhält sich genauso.",
    set_key: "AES-Schlüssel festlegen",
    key_needs: "Der Editor braucht den pak-AES-Schlüssel von Echoes of Aincrad, um deinen Spielstand zu lesen. Füge ihn unten ein oder hole ihn aus dem laufenden Spiel.",
    key_recover: "Aus laufendem Spiel holen", key_scanning: "Suche…", save_key: "Schlüssel speichern",
    change_key: "Schlüssel ändern",
    key_hint: "Keinen Schlüssel? Starte Echoes of Aincrad, betritt die Welt und klicke auf „Aus laufendem Spiel holen“.",
    key_not_ship: "Wir liefern den Schlüssel nie mit — er wird aus deinem eigenen Spiel gelesen.",
    looks_intro: "Speichere das Aussehen dieses Charakters als benannten Look oder wende einen an. Looks sind deine eigenen Backups, getrennt vom Spielstand.",
    save_look_as: "Aktuellen Look speichern", save_look: "Look speichern", saved_looks: "Gespeicherte Looks",
    no_looks: "Noch keine Looks gespeichert.", apply: "Anwenden", delete: "Löschen",
    open_to_begin: "Öffne einen Spielstand, um zu beginnen.",
    update_available: "Update verfügbar", update_hint: "Eine neuere Version des Editors ist verfügbar — klicken, um die Download-Seite zu öffnen.",
};

const ES: S = S {
    open_save: "Abrir partida…", save: "Guardar", apply_to_game: "Aplicar al juego…", reload: "Recargar",
    working_copy: "COPIA DE TRABAJO — partida real a salvo", unsaved: "sin guardar",
    thumbnails: "Miniaturas", character: "Personaje", copy_diagnostics: "Copiar diagnóstico",
    language: "Idioma",
    cat_identity: "Identidad", cat_face: "Cara", cat_hair: "Pelo y extras", cat_body: "Cuerpo", cat_looks: "Looks",
    cat_backups: "Respaldos",
    backups_intro: "Cada Guardar o Aplicar conserva un respaldo con fecha y hora: de la copia de trabajo en la carpeta del editor, y del guardado del juego junto a su archivo. Carga uno para inspeccionarlo o restaurarlo: cargar solo llena la copia de trabajo; tu guardado real no se toca hasta que Apliques.",
    backup_load: "Cargar", no_backups: "Aún no hay respaldos; aparecen tras el primer Guardar o Aplicar.",
    backup_live: "guardado del juego", backup_work: "copia de trabajo",
    name: "Nombre", gender: "Género", male: "Hombre", female: "Mujer", voice: "Voz",
    voice_any_body: "Las voces no están ligadas al género: cualquier voz funciona con cualquier cuerpo.",
    voice_preview: "Reproducir una muestra de esta voz; cada pulsación reproduce una línea distinta.",
    mode_title: "Modo de juego",
    death_game: "Death game (muerte permanente)",
    death_game_note: "La opción de muerte permanente elegida al crear el personaje. Activado: la muerte de este personaje es definitiva. Desactivado: muerte normal. Se aplica la próxima vez que el juego cargue la partida.",
    nose: "Nariz", eyebrows: "Cejas", eye_shape: "Forma de ojos", eyes_iris: "Ojos / iris", head_shape: "Forma de cabeza",
    hair: "Pelo", mole: "Lunar", freckles: "Pecas", none: "Ninguno",
    colours: "Colores",
    game_palette: "Colores del juego",
    game_palette_hint: "Elige de la paleta del creador de personajes: haz clic en una muestra para igualar el color del juego.",
    body_hidden: "Los deslizadores de cuerpo están limitados al rango seguro del juego, para no deformar tu personaje.",
    body_neck_note: "El control de pecho también remodela la base del cuello: el juego no tiene control de cuello separado; el creador del juego hace lo mismo.",
    set_key: "Configura tu clave AES",
    key_needs: "El editor necesita la clave AES pak de Echoes of Aincrad para leer tu partida. Pégala abajo o recupérala de tu juego en ejecución.",
    key_recover: "Recuperar del juego", key_scanning: "Analizando…", save_key: "Guardar clave",
    change_key: "Cambiar clave",
    key_hint: "¿No tienes la clave? Inicia Echoes of Aincrad, entra al mundo y pulsa «Recuperar del juego».",
    key_not_ship: "Nunca distribuimos la clave: se lee de tu propio juego.",
    looks_intro: "Guarda la apariencia de este personaje como un look con nombre, o aplica uno guardado. Los looks son tus copias, aparte de la partida.",
    save_look_as: "Guardar look actual", save_look: "Guardar look", saved_looks: "Looks guardados",
    no_looks: "Aún no hay looks guardados.", apply: "Aplicar", delete: "Eliminar",
    open_to_begin: "Abre una partida para empezar.",
    update_available: "Actualización disponible", update_hint: "Hay una versión más reciente del editor — haz clic para abrir la página de descargas.",
};

const PT: S = S {
    open_save: "Abrir save…", save: "Salvar", apply_to_game: "Aplicar ao jogo…", reload: "Recarregar",
    working_copy: "CÓPIA DE TRABALHO — save real protegido", unsaved: "não salvo",
    thumbnails: "Miniaturas", character: "Personagem", copy_diagnostics: "Copiar diagnóstico",
    language: "Idioma",
    cat_identity: "Identidade", cat_face: "Rosto", cat_hair: "Cabelo e extras", cat_body: "Corpo", cat_looks: "Looks",
    cat_backups: "Backups",
    backups_intro: "Cada Salvar ou Aplicar guarda um backup com data e hora — da cópia de trabalho na pasta do editor, e do save do jogo ao lado do arquivo dele. Carregue um para inspecionar ou restaurar: carregar só preenche a cópia de trabalho; seu save real não é tocado até você Aplicar.",
    backup_load: "Carregar", no_backups: "Nenhum backup ainda — eles aparecem após o primeiro Salvar ou Aplicar.",
    backup_live: "save do jogo", backup_work: "cópia de trabalho",
    name: "Nome", gender: "Gênero", male: "Masculino", female: "Feminino", voice: "Voz",
    voice_any_body: "As vozes não são presas ao gênero — qualquer voz funciona em qualquer corpo.",
    voice_preview: "Tocar uma amostra desta voz — cada toque reproduz uma fala diferente.",
    mode_title: "Modo de jogo",
    death_game: "Death game (morte permanente)",
    death_game_note: "A escolha de morte permanente feita na criação do personagem. Ligado: a morte deste personagem é definitiva. Desligado: morte normal. Vale a partir do próximo carregamento do save.",
    nose: "Nariz", eyebrows: "Sobrancelhas", eye_shape: "Formato dos olhos", eyes_iris: "Olhos / íris", head_shape: "Formato do rosto",
    hair: "Cabelo", mole: "Pinta", freckles: "Sardas", none: "Nenhum",
    colours: "Cores",
    game_palette: "Cores do jogo",
    game_palette_hint: "Escolha da paleta do criador de personagem — clique numa amostra para igualar a cor do jogo.",
    body_hidden: "Os controles de corpo são limitados à faixa segura do jogo, para não deformar seu personagem.",
    body_neck_note: "O controle de peito também remodela a base do pescoço — o jogo não tem controle de pescoço separado; o criador do jogo faz o mesmo.",
    set_key: "Defina sua chave AES",
    key_needs: "O editor precisa da chave AES pak de Echoes of Aincrad para ler seu save. Cole abaixo ou recupere do seu jogo em execução.",
    key_recover: "Recuperar do jogo", key_scanning: "Analisando…", save_key: "Salvar chave",
    change_key: "Alterar chave",
    key_hint: "Não tem a chave? Abra Echoes of Aincrad, entre no mundo e clique em \"Recuperar do jogo\".",
    key_not_ship: "Nunca distribuímos a chave: ela é lida do seu próprio jogo.",
    looks_intro: "Salve a aparência deste personagem como um look nomeado, ou aplique um salvo. Looks são seus backups, separados do save do jogo.",
    save_look_as: "Salvar look atual", save_look: "Salvar look", saved_looks: "Looks salvos",
    no_looks: "Nenhum look salvo ainda.", apply: "Aplicar", delete: "Excluir",
    open_to_begin: "Abra um save para começar.",
    update_available: "Atualização disponível", update_hint: "Há uma versão mais recente do editor — clique para abrir a página de downloads.",
};

const IT: S = S {
    open_save: "Apri salvataggio…", save: "Salva", apply_to_game: "Applica al gioco…", reload: "Ricarica",
    working_copy: "COPIA DI LAVORO — salvataggio reale al sicuro", unsaved: "non salvato",
    thumbnails: "Miniature", character: "Personaggio", copy_diagnostics: "Copia diagnostica",
    language: "Lingua",
    cat_identity: "Identità", cat_face: "Viso", cat_hair: "Capelli & Extra", cat_body: "Corpo", cat_looks: "Look",
    cat_backups: "Backup",
    backups_intro: "Ogni Salva o Applica conserva un backup con data e ora — della copia di lavoro nella cartella dell'editor, e del salvataggio del gioco accanto al suo file. Caricane uno per ispezionarlo o ripristinarlo: il caricamento riempie solo la copia di lavoro; il salvataggio reale resta intatto finché non Applichi.",
    backup_load: "Carica", no_backups: "Nessun backup ancora — compaiono dopo il primo Salva o Applica.",
    backup_live: "salvataggio del gioco", backup_work: "copia di lavoro",
    name: "Nome", gender: "Genere", male: "Uomo", female: "Donna", voice: "Voce",
    voice_any_body: "Le voci non sono legate al genere — qualsiasi voce funziona su qualsiasi corpo.",
    voice_preview: "Riproduci un campione di questa voce — ogni pressione riproduce una battuta diversa.",
    mode_title: "Modalità di gioco",
    death_game: "Death game (morte permanente)",
    death_game_note: "La scelta di morte permanente fatta alla creazione del personaggio. Attivo: la morte di questo personaggio è definitiva. Disattivo: morte normale. Vale dal prossimo caricamento del salvataggio.",
    nose: "Naso", eyebrows: "Sopracciglia", eye_shape: "Forma occhi", eyes_iris: "Occhi / iride", head_shape: "Forma del viso",
    hair: "Capelli", mole: "Neo", freckles: "Lentiggini", none: "Nessuno",
    colours: "Colori",
    game_palette: "Colori del gioco",
    game_palette_hint: "Scegli dalla palette del creatore del personaggio: clicca un campione per usare il colore del gioco.",
    body_hidden: "I cursori del corpo sono limitati all'intervallo sicuro del gioco, per non deformare il personaggio.",
    body_neck_note: "Il cursore del petto rimodella anche la base del collo — il gioco non ha un controllo separato per il collo; il creatore del gioco fa lo stesso.",
    set_key: "Imposta la tua chiave AES",
    key_needs: "L'editor ha bisogno della chiave AES pak di Echoes of Aincrad per leggere il salvataggio. Incollala sotto o recuperala dal gioco in esecuzione.",
    key_recover: "Recupera dal gioco", key_scanning: "Scansione…", save_key: "Salva chiave",
    change_key: "Cambia chiave",
    key_hint: "Non hai la chiave? Avvia Echoes of Aincrad, entra nel mondo e clicca \"Recupera dal gioco\".",
    key_not_ship: "Non distribuiamo mai la chiave: viene letta dal tuo gioco.",
    looks_intro: "Salva l'aspetto di questo personaggio come look con nome, o applicane uno salvato. I look sono i tuoi backup, separati dal salvataggio.",
    save_look_as: "Salva look attuale", save_look: "Salva look", saved_looks: "Look salvati",
    no_looks: "Nessun look salvato.", apply: "Applica", delete: "Elimina",
    open_to_begin: "Apri un salvataggio per iniziare.",
    update_available: "Aggiornamento disponibile", update_hint: "È disponibile una versione più recente dell'editor — clicca per aprire la pagina dei download.",
};

const RU: S = S {
    open_save: "Открыть сохранение…", save: "Сохранить", apply_to_game: "Применить к игре…", reload: "Перезагрузить",
    working_copy: "РАБОЧАЯ КОПИЯ — сохранение в безопасности", unsaved: "не сохранено",
    thumbnails: "Миниатюры", character: "Персонаж", copy_diagnostics: "Копировать диагностику",
    language: "Язык",
    cat_identity: "Профиль", cat_face: "Лицо", cat_hair: "Волосы и прочее", cat_body: "Тело", cat_looks: "Образы",
    cat_backups: "Бэкапы",
    backups_intro: "Каждое сохранение или применение создаёт бэкап с меткой времени — рабочей копии в папке редактора и игрового сейва рядом с его файлом. Загрузите бэкап, чтобы просмотреть или восстановить его: загрузка заполняет только рабочую копию; настоящий сейв не меняется, пока вы не нажмёте «Применить».",
    backup_load: "Загрузить", no_backups: "Бэкапов пока нет — они появятся после первого сохранения или применения.",
    backup_live: "игровой сейв", backup_work: "рабочая копия",
    name: "Имя", gender: "Пол", male: "Мужской", female: "Женский", voice: "Голос",
    voice_any_body: "Голоса не привязаны к полу — любой голос подходит любому телу.",
    voice_preview: "Прослушать образец этого голоса — каждое нажатие проигрывает другую реплику.",
    mode_title: "Режим игры",
    death_game: "Смертельная игра (пермадет)",
    death_game_note: "Выбор пермадета при создании персонажа. Вкл.: смерть этого персонажа окончательна. Выкл.: обычная смерть. Действует со следующей загрузки сохранения.",
    nose: "Нос", eyebrows: "Брови", eye_shape: "Форма глаз", eyes_iris: "Глаза / радужка", head_shape: "Форма головы",
    hair: "Волосы", mole: "Родинка", freckles: "Веснушки", none: "Нет",
    colours: "Цвета",
    game_palette: "Цвета игры",
    game_palette_hint: "Выберите из палитры редактора персонажа — щёлкните образец, чтобы взять цвет игры.",
    body_hidden: "Ползунки телосложения ограничены безопасным диапазоном игры, чтобы не испортить модель.",
    body_neck_note: "Ползунок груди также меняет форму основания шеи — отдельной настройки шеи в игре нет; игровой редактор ведёт себя так же.",
    set_key: "Задайте AES-ключ",
    key_needs: "Редактору нужен pak AES-ключ Echoes of Aincrad для чтения сохранения. Вставьте его ниже или получите из запущенной игры.",
    key_recover: "Получить из игры", key_scanning: "Сканирование…", save_key: "Сохранить ключ",
    change_key: "Изменить ключ",
    key_hint: "Нет ключа? Запустите Echoes of Aincrad, войдите в мир и нажмите «Получить из игры».",
    key_not_ship: "Мы не распространяем ключ — он считывается из вашей собственной игры.",
    looks_intro: "Сохраните внешность персонажа как именованный образ или примените сохранённый. Образы — ваши личные копии, отдельно от сохранения игры.",
    save_look_as: "Сохранить текущий образ", save_look: "Сохранить образ", saved_looks: "Сохранённые образы",
    no_looks: "Пока нет сохранённых образов.", apply: "Применить", delete: "Удалить",
    open_to_begin: "Откройте сохранение, чтобы начать.",
    update_available: "Доступно обновление", update_hint: "Доступна более новая версия редактора — нажмите, чтобы открыть страницу загрузки.",
};

const ZH: S = S {
    open_save: "打开存档…", save: "保存", apply_to_game: "应用到游戏…", reload: "重新加载",
    working_copy: "工作副本 — 原存档安全", unsaved: "未保存",
    thumbnails: "缩略图", character: "角色", copy_diagnostics: "复制诊断信息",
    language: "语言",
    cat_identity: "基本", cat_face: "脸部", cat_hair: "发型与其他", cat_body: "身体", cat_looks: "外观",
    cat_backups: "备份",
    backups_intro: "每次保存或应用都会留下带时间戳的备份——工作副本的备份在编辑器文件夹里，游戏存档的备份在存档文件旁边。载入一个即可查看或恢复：载入只写入工作副本；在你点击“应用到游戏”之前，真实存档不会被改动。",
    backup_load: "载入", no_backups: "还没有备份——首次保存或应用后就会出现。",
    backup_live: "游戏存档", backup_work: "工作副本",
    name: "名字", gender: "性别", male: "男性", female: "女性", voice: "声音",
    voice_any_body: "声音不与性别绑定——任何声音都可用于任何身体。",
    voice_preview: "播放该声音的示例——每次点击播放不同台词。",
    mode_title: "游戏模式",
    death_game: "死亡游戏（永久死亡）",
    death_game_note: "创建角色时的永久死亡选项。开启：该角色的死亡将是最终结局。关闭：普通死亡。下次游戏读取存档时生效。",
    nose: "鼻子", eyebrows: "眉毛", eye_shape: "眼型", eyes_iris: "眼睛 / 虹膜", head_shape: "脸型",
    hair: "头发", mole: "痣", freckles: "雀斑", none: "无",
    colours: "颜色",
    game_palette: "游戏配色",
    game_palette_hint: "从角色创建器自带的调色板中选择——点击色块即可匹配游戏颜色。",
    body_hidden: "体型滑块已限制在游戏的安全范围内，不会让角色模型变形。",
    body_neck_note: "胸部滑块也会改变脖颈根部的形状——游戏没有单独的脖子设置，游戏内捏人界面也是如此。",
    set_key: "设置你的 AES 密钥",
    key_needs: "编辑器需要 Echoes of Aincrad 的 pak AES 密钥来读取存档。请在下方粘贴，或从运行中的游戏获取。",
    key_recover: "从运行中的游戏获取", key_scanning: "扫描中…", save_key: "保存密钥",
    change_key: "更改密钥",
    key_hint: "没有密钥？启动 Echoes of Aincrad，进入游戏世界，然后点击“从运行中的游戏获取”。",
    key_not_ship: "我们从不附带密钥 — 这会从你自己的游戏中读取你自己的密钥。",
    looks_intro: "将该角色的外观保存为命名外观，或应用已保存的外观。外观是你自己的备份，与游戏存档分开。",
    save_look_as: "保存当前外观", save_look: "保存外观", saved_looks: "已保存外观",
    no_looks: "还没有保存的外观。", apply: "应用", delete: "删除",
    open_to_begin: "打开存档以开始。",
    update_available: "有可用更新", update_hint: "编辑器有更新版本可用——点击打开下载页面。",
};

const KO: S = S {
    open_save: "세이브 열기…", save: "저장", apply_to_game: "게임에 적용…", reload: "다시 불러오기",
    working_copy: "작업 사본 — 실제 세이브 안전", unsaved: "저장 안 됨",
    thumbnails: "썸네일", character: "캐릭터", copy_diagnostics: "진단 정보 복사",
    language: "언어",
    cat_identity: "기본", cat_face: "얼굴", cat_hair: "머리 & 기타", cat_body: "몸", cat_looks: "룩",
    cat_backups: "백업",
    backups_intro: "저장하거나 적용할 때마다 타임스탬프가 붙은 백업이 남습니다. 작업 사본 백업은 에디터 폴더에, 게임 세이브 백업은 세이브 파일 옆에 저장됩니다. 백업을 불러와 확인하거나 복원할 수 있으며, 불러오기는 작업 사본만 채웁니다. '게임에 적용' 전까지 실제 세이브는 바뀌지 않습니다.",
    backup_load: "불러오기", no_backups: "아직 백업이 없습니다. 첫 저장 또는 적용 후에 나타납니다.",
    backup_live: "게임 세이브", backup_work: "작업 사본",
    name: "이름", gender: "성별", male: "남성", female: "여성", voice: "음성",
    voice_any_body: "음성은 성별에 고정되지 않습니다. 어떤 몸에도 원하는 음성을 쓸 수 있습니다.",
    voice_preview: "이 음성의 샘플을 재생합니다. 누를 때마다 다른 대사가 재생됩니다.",
    mode_title: "게임 모드",
    death_game: "데스 게임(영구 사망)",
    death_game_note: "캐릭터 생성 시의 영구 사망 선택입니다. 켬: 이 캐릭터의 죽음은 되돌릴 수 없습니다. 끔: 일반 사망. 다음에 게임이 이 세이브를 불러올 때부터 적용됩니다.",
    nose: "코", eyebrows: "눈썹", eye_shape: "눈 모양", eyes_iris: "눈 / 홍채", head_shape: "머리 모양",
    hair: "머리", mole: "점", freckles: "주근깨", none: "없음",
    colours: "색상",
    game_palette: "게임 색상",
    game_palette_hint: "캐릭터 생성기 팔레트에서 선택 — 색상 견본을 클릭하면 게임 색상과 맞춰집니다.",
    body_hidden: "체형 슬라이더는 게임의 안전 범위로 제한되어 모델이 손상되지 않습니다.",
    body_neck_note: "가슴 슬라이더는 목 아랫부분 형태에도 영향을 줍니다. 게임에 별도의 목 설정은 없으며, 게임 내 캐릭터 메이킹도 동일합니다.",
    set_key: "AES 키 설정",
    key_needs: "세이브를 읽으려면 Echoes of Aincrad의 pak AES 키가 필요합니다. 아래에 붙여넣거나 실행 중인 게임에서 가져오세요.",
    key_recover: "실행 중인 게임에서 가져오기", key_scanning: "검색 중…", save_key: "키 저장",
    change_key: "키 변경",
    key_hint: "키가 없나요? Echoes of Aincrad를 실행해 월드에 들어간 뒤 \"실행 중인 게임에서 가져오기\"를 누르세요.",
    key_not_ship: "키는 배포하지 않습니다. 본인의 게임에서 본인의 키를 읽습니다.",
    looks_intro: "이 캐릭터의 외형을 이름 붙인 룩으로 저장하거나 저장된 룩을 적용하세요. 룩은 게임 세이브와 별개인 개인 백업입니다.",
    save_look_as: "현재 룩 저장", save_look: "룩 저장", saved_looks: "저장된 룩",
    no_looks: "저장된 룩이 없습니다.", apply: "적용", delete: "삭제",
    open_to_begin: "세이브를 열어 시작하세요.",
    update_available: "업데이트 있음", update_hint: "에디터의 새 버전이 있습니다. 클릭하면 다운로드 페이지가 열립니다.",
};
